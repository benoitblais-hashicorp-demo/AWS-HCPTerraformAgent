...
Thank you for your interest in contributing. This repository uses Terraform to provision AWS infrastructure for HCP Terraform agents. The target architecture is one EC2 host running 5 HCP Terraform agent processes. Please review these guidelines before contributing.
...
3. **Preserve Architecture Intent:** Keep changes aligned to one EC2 host and 5 HCP Terraform agents unless the specification is intentionally updated.
4. **Format:** Formatting checks are enforced by CI/CD. Local formatting is optional for contributor convenience.
5. **Open a Pull Request:** Fill out the provided PR template outlining your changes.
